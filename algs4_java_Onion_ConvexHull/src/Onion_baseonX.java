...
public static Point2D[] convex_hull(Point2D[] P) {
        if (P.length > 1) {
                ...
                for(int i=0;i<n;i++) temp[i]=new Point2D(P[i].y(),P[i].x());
                Arrays.sort(temp);//the smallest y will be first and next
                for(int i=0;i<n;i++) P[i]=new Point2D(temp[i].y(),temp[i].x());
